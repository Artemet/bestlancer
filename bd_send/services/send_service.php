...
if (empty($service_name) || empty($service_information) || empty($service_price)) {
    ...
    $sql = "INSERT INTO `services` (`id`, `category`, `file_path`, `name`, `information`, `price`, `eye`, `nik`) VALUES (NULL, ?, ?, ?, ?, ?, 0, ?)";
    $stmt = mysqli_prepare($bd_connect, $sql);
    mysqli_stmt_bind_param($stmt, "isssis", $index, $file_name, $service_name, $service_information, $service_price, $user_nik);
