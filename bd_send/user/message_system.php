...
if (isset($_SESSION["nik"])) {
    ...
    function task_interval()
    {
        ...
        $user_sql = "SELECT * FROM `user_registoring` WHERE `nik` = '$message_nik'";
        $user_query = mysqli_query($bd_connect, $user_sql);
        $order_sql = "SELECT * FROM `orders` WHERE `progress` = 2 AND `nik` = '$nik' OR `nik` = '$message_nik'";
    ...
    //task_interval();
    $sql = "INSERT INTO `messages` (`id`, `chat_id`, `date`, `message_value`, `file_path`, `time`, `eye`, `changeable`, `deleted`, `answer`, `task`, `file`, `nik`, `message_nik`) VALUES (NULL, '$chat_id', '$formattedDate', '$message_value', '$file_name', '$message_time', 0, 0, 0, $answer_id, 0, $file_information, '$nik', '$message_nik')";
