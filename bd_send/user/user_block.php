...
if (isset($_GET['user_id']) && is_numeric($_GET['user_id'])) {
    ...
    $user_status_sql = "SELECT `status` FROM `messenger_users` WHERE `nik_one` = '$my_nik' AND `nik_two` = '$blocking_user_resolt' OR `nik_two` = '$my_nik' AND `nik_one` = '$blocking_user_resolt'";
    $user_block_sql = "SELECT `main_block` FROM `messenger_users` WHERE `nik_one` = '$my_nik' AND `nik_two` = '$blocking_user_resolt' OR `nik_two` = '$my_nik' AND `nik_one` = '$blocking_user_resolt'";
    ...
    if ($user_status_resolt == 'unblock') {
        $block_sql = "UPDATE `messenger_users` SET `status` = 'block', `main_block` = '$my_nik' WHERE `nik_one` = '$my_nik' AND `nik_two` = '$blocking_user_resolt' OR `nik_two` = '$my_nik' AND `nik_one` = '$blocking_user_resolt'";
    } elseif ($user_status_resolt == 'block') {
        $block_sql = "UPDATE `messenger_users` SET `status` = 'unblock', `main_block` = '' WHERE `nik_one` = '$my_nik' AND `nik_two` = '$blocking_user_resolt' OR `nik_two` = '$my_nik' AND `nik_one` = '$blocking_user_resolt'";
    }
