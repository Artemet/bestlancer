...
function order_include() {
  ...
  get_arrow.forEach((item) => {
    item.addEventListener("click", function () {
      ...
      get_object
        .querySelector(".object_sub")
        .classList.toggle("object_sub_active");
    });
  });
}
order_include();
...
function button_solution() {
  ...
  const get_button = document.querySelectorAll(
    ".order_progress .button_option button"
  );
  const get_order_id = document
    .querySelector("noscript.order_id")
    .innerHTML.trim();
  get_button.forEach((item) => {
    item.addEventListener("click", function () {
      ...
      if (button_id === 0) {
        ...
        action =
          "Вы действительно подтверждаете пришедшую сумму на ваш кошелёк и готовы работать дальше с заказчиком?";
      } else if (button_id === 7) {
        action =
          "Вы выполнили этап работы, и заказчик сказал, что оплатил вам работу. Вы уверины, что средства не пришли на ваши реквезиты?";
      } else {
      ...
      if (ask || order_pause === true) {
        ...
        const done_order = () => {
          ...
          const modal_logic = () => {
            const get_button = document.querySelector(
              ".finish_order_container button"
            );

            //checkbox_click
            let checkbox_temp = 0;
            const get_checkbox_line = document.querySelector(
              ".finish_order_container .checkbox_agree"
            );
            ...
            //modal_close
            const get_cross = document.querySelector(
              ".finish_order_container .close"
            );
            ...
            get_button.addEventListener("click", function () {
              const get_message = document
                .querySelector(".finish_order_container textarea")
                .value.trim();
              $.ajax({
                method: "POST",
                url:
                  "../bd_send/order/order_action.php?action_order_id=" +
                  get_order_id,
                ...
                get_modal
                  .querySelectorAll(".overlay")[0]
                  .classList.add("overlay_active");
                get_modal
                  .querySelector(".finish_wrapper")
                  .classList.add("finish_wrapper_load");
                setTimeout(() => {
        ...
        const payment_ask = () => {
          ...
          //value_check
          const get_button = document.querySelector(
            ".payment_ask_container button"
          );
          const get_input = document.querySelector(
            ".payment_ask_container input.right_in"
          );
          ...
          get_button.addEventListener("click", function () {
            const get_warning = document.querySelector(
              ".payment_ask_container u.warning"
            );
            ...
            $.ajax({
              ...
              const get_overlay = document.querySelector(
                ".payment_ask_container .overlay"
              );
              const get_wrapper = document.querySelector(
                ".payment_ask_container .payment_ask"
              );
              if (response === "limit") {
        ...
        $.ajax({
          method: "POST",
          url:
            "../bd_send/order/order_action.php?action_order_id=" + get_order_id,
...
function review_write() {
  ...
  if (get_container !== null) {
    ...
    //smile_click
    const get_smile_wrappers = document.querySelectorAll(
      ".review_write .smiles div"
    );
    ...
    get_textarea.addEventListener("input", function () {
      const get_length_tag = document.querySelector(
        ".review_write .length_controll span"
      );
    ...
    get_value_button.addEventListener("click", function () {
      ...
      if (get_textarea.value.length <= 49) {
        ...
        $.ajax({
          method: "POST",
          url:
            "../bd_send/order/order_action.php?review_order_id=" + get_order_id,
          data: { smile_index: active_smile, review: review_value },
        }).done(function (response) {
          console.log(response);
          get_loader.style.display = "flex";
          document
            .querySelector(".order_progress .review_wrapper")
            .classList.add("review_wrapper_load");
          get_value_button.classList.add("none_ready");
          setTimeout(() => {
...
function user_pay() {
  ...
  const get_order_id = document
    .querySelector("noscript.order_id")
    .innerHTML.trim();
  const get_sum = document
    .querySelector("noscript.sum_amount")
    .innerHTML.trim();
  ...
  const requisitions_check = () => {
    let requisition_arr = [];
    get_requisitions.forEach((item) => {
      const none_requisition = item.querySelector(".none");
      if (none_requisition === null) {
        requisition_arr.push(false);
      } else {
        requisition_arr.push(true);
      }
    });
    if (requisition_arr.includes(true)) {
