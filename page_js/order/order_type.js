...
function order_type(){
    ...
    document.addEventListener('DOMContentLoaded', function (){
      ...
      let sub_heights = [];
      for (let i = 0; i < document.querySelectorAll(".main_category").length; i++){
      ...
      get_links.forEach( (item) => {
        item.addEventListener('click', (e) => {
          if (!item.parentNode.className.includes("none_use")){
            const get_link_name = document.querySelectorAll(".tasks_container .make_order .link_part p");
            if (item.parentNode.parentNode.className.includes("link_part")){
              const get_filter_none_link = document.querySelector(".tasks_container .tasks a.filter_none");
            ...
            if (!item.closest("div").className.includes("reaload_icon")){
              ...
              const get_active_menus = document.querySelectorAll(".tasks_container .make_order .link_part p");
              get_active_menus.forEach( (item) => {
                if (item.className.includes("active")){
                  setTimeout( () => {
                    item.closest("div").querySelector(".link_sub").style.height = sub_heights[item.closest("a").id];
                  }, 1);
                }
              });
            }
            ...
            if (!item.className.includes("filter_none") && !item.closest("div").className.includes("reaload_icon")){
              get_subs[item.id].style.height = sub_heights[item.id];
            ...
            const get_active_menu = document.querySelectorAll(".tasks_container .make_order .link_part p");
