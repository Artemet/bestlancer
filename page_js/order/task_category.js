function category_hrefs() {
  let href_temp = 0;
  const get_medium_links = document.querySelectorAll(
    ".tasks_container .make_order .link_sub a.medium_category"
  );
  for (let i = 0; i < get_medium_links.length; i++) {
    href_temp++;
    get_medium_links[i].href = "?medium_category=" + href_temp;
  }
}
category_hrefs();
...
function task_category() {
  let href_temp = 0;
  const get_filter_link = document.querySelectorAll(
    ".tasks_container .tasks a.filter_none"
  );
  const get_link_sub = document.querySelectorAll(
    ".tasks_container .make_order .link_sub"
  );
  ...
  if (get_filter_link.length >= 1) {
    const get_tasks_category = document.querySelectorAll(
      ".tasks_container .category b"
    );
    const get_category_options = document.querySelectorAll(
  ...
  //medium_category
  const get_medium_category_number = document.querySelector(
    ".tasks_container .medium_category_number"
  );
  const resolt_number = get_medium_category_number.textContent.replace(
    /\s+/g,
    ""
  );
  if (!/^\s*$/.test(resolt_number)) {
    const get_order = document.querySelectorAll(".tasks_container .category b");
    const get_category_name = document.querySelectorAll(
      ".tasks_container .make_order .link_part p"
    );
    const get_medium_number =
      document.querySelector("p.medium_number").innerHTML;
    ...
    if (get_order.length >= 1) {
      get_category_name.forEach((item) => {
        if (item.innerHTML.includes(get_order[0].innerHTML.trim())) {
          const parent = item.closest("div");
          const get_sub = parent.querySelector(".link_sub");
          const get_medium_sub = document.querySelectorAll(
            ".tasks_container .final_filter_sub"
          );
          item.classList.add("active");
          get_sub.classList.add("sub_active");
          get_medium_sub[medium_number_convert].classList.add("sub_active");
        }
      });
    }
  }
  //final_category
  const get_final_category_number = document.querySelector(
    ".tasks_container .final_number"
  );
  if (get_final_category_number !== null) {
    const final_number = parseInt(get_final_category_number.innerHTML, 10) - 1;
    const get_final_link = document.querySelectorAll(
      ".tasks_container .link_sub .final_filter_sub a"
    )[final_number];
  ...
  function save_category() {
    const get_save_icon = document.querySelector(
      ".tasks_container .icon_options .icon"
    );
