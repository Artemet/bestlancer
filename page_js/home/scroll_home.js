...
function scroll_zero() {
  let number_temp = -1;
  const get_number = document.querySelectorAll(
    ".freelance_container .movement_number .movement span"
  );
  setTimeout(() => {
    setInterval(() => {
      number_temp++;
...
function repitation_scroll() {
  setTimeout(() => {
    let number_temp = -1;
    const get_reoetition_number_block = document.querySelectorAll(
      ".repitation_container .main_reoitation .number"
    );
    const get_reoetition_number_b = document.querySelectorAll(
      ".repitation_container .main_reoitation b"
    );
    ...
    const get_reoetition_number = document.querySelectorAll(
      ".repitation_container .main_reoitation span.active"
    );
    get_reoetition_number_b.forEach((element) => {
      element.textContent = element.textContent.replace(/\s/g, "");
    });
    let resolt_numbers = [
      get_reoetition_number_b[0].innerHTML,
      get_reoetition_number_b[1].innerHTML,
      get_reoetition_number_b[2].innerHTML,
    ];
    setInterval(() => {
      number_temp++;
      for (let i = 0; i < 3; i++) {
        ...
        if (get_reoetition_number[i].innerHTML == resolt_numbers[i]) {
          ...
          setTimeout(() => {
            get_reoetition_number_block[i].innerHTML =
              "<b style='display: block;'>" + resolt_numbers[i] + "</b>";
          }, 5000);
        }
      }
    }, 1);
...
window.addEventListener("scroll", function () {
  ...
  function question_scroll() {
    let anser_temp = -1;
    const get_question_block = document.querySelector(
      ".about_us .why_question"
    );
