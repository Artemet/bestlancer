...
function retes_menu(){
    ...
    get_options.forEach( (item) => {
        sub_heights.push(item.closest(".option_wrapper").querySelector(".option_sub").clientHeight + "px");
        item.closest(".option_wrapper").querySelector(".option_sub").style.height = 0;
        item.addEventListener("click", function (){
            ...
            document.querySelectorAll(".option_wrapper").forEach( (item) => {
                if (item.className.includes("sub_animation")){
                    ...
                    document.querySelectorAll(".rates_container .option svg")[item_id].classList.add("animation");
                } else{
                    item.querySelector(".option_sub").style.height = 0;
                    item.closest(".option_wrapper").querySelector("svg").classList.remove("animation");
                }
            });
        });
    });
...
function retes_price(){
    const get_checkbox_wrapper = document.querySelectorAll(".rates_container .option_sub div");
    const get_final_price = document.querySelector(".rates_container .price b.price");
    get_checkbox_wrapper.forEach( (item) => {
        item.addEventListener("click", function (){
            item.classList.toggle("active_block");
            const get_active_blocks = document.querySelectorAll(".rates_container .option_sub .active_block");
            ...
            let resolt_formula = 3 * get_active_blocks.length;
            get_final_price.innerHTML = resolt_formula;
            if (!item.className.includes("active_block")){
                item.querySelector("input").checked = false;
                item.closest(".option_sub").querySelectorAll("div").forEach( (item) => {
                    item.classList.remove("none_click")
                });
                console.log(item);
                if (item.className.includes("special_choice")){
                    item.closest(".option_sub").querySelectorAll("input").forEach( (item) => {
                        item.checked = false;
                    });
                }
            } else{
