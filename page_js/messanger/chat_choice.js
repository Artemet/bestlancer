...
//chat_choice_ajax
const get_companion_infromation =
  document.querySelectorAll(".companion_choice").length;
//text_hide
function text_hide() {
  const get_values = document.querySelectorAll(
    ".chat_row .message p.message_value"
  );
    ...
    if (text.length >= 1000) {
      if (element.parentNode.hasAttribute("class")) {
        element.innerHTML =
          ...
          '... <b class="more_text text_function" id="0">Далее</b>';
      } else {
      ...
      function more_text() {
        ...
        more_text.forEach((item) => {
          item.addEventListener("click", function () {
            ...
            if (item_id === 0) {
              element.innerHTML =
                text + '<b class="less_text text_function" id="1"> Скрыть</b>';
              less_text();
            } else {
              element.innerHTML =
                text.substring(0, 1000) +
                '... <b class="more_text" id="0">Далее</b>';
            }
          });
        });
      }
...
document.addEventListener("DOMContentLoaded", function () {
      ...
      .catch((error) => {
        ...
        menu_icon.addEventListener("click", function () {
          ...
          const get_sub_menu = document.querySelector(
            ".user_chat .more_options .sub_menu"
          );
  ...
  const setupEventListeners = () => {
        ...
        message_option.forEach((item) => {
          item.addEventListener("click", function () {
            ...
            if (item_class.includes("delete")) {
              ...
              final_option = "change";
              const message_value = item
                .closest(".chat_row .message")
                .querySelector("p.main_value")
                .textContent.trim();
              const get_message_input = document.querySelector(
                ".user_chat input.message_right"
              );
              ...
              get_message_input.addEventListener("input", function () {
                if (get_message_input.value === "") {
                  const get_chat_id = parseInt(
                    document.querySelector(".chat_number").innerHTML.trim(),
                    10
                  );
                  get_message_input.removeAttribute("id");
                  get_button.innerHTML = "Отправить";
                  request_link =
                    "../bd_send/user/message_system.php?chat_id=" + get_chat_id;
                }
              });
            }
            if (final_option === "delete") {
              $.ajax({
                ...
                if (final_option === "delete") {
                  document
                    .querySelector(".messanger_users .message_user_active")
                    .click();
                }
              });
            }
    ...
    $(".user_chat button").on("click", function () {
      ...
      let message_id = document
        .querySelector(".user_chat input.message_right")
        .id.trim();
      ...
      const get_files = document.querySelectorAll(
        ".user_chat .file_line .file"
      );
  ...
  get_chats.forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      const get_notification_information = item.querySelectorAll(
        ".notification_number"
      );
        ...
        setTimeout(() => {
          if (
            document.querySelector(
              ".user_chat .companion_options a.reload_page_icon"
            ) !== null
          ) {
            document.querySelector(
              ".user_chat .companion_options a.reload_page_icon"
      ...
      document
        .querySelectorAll(".messanger_users .message_user")
        .forEach((item) => {
